 ...
 if(in_array($extension, $allowed_extension)) //check selected file extension is present in allowed extension array
 {
  ...
  foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
  {
   ...
   for($row=2; $row<=$highestRow; $row++)
   {
    ...
    {
      $query = $db->query("INSERT INTO referanslar(kategori,projeadi,ili,urun,uygulama,miktar,lang,IsActive) VALUES ('".$kategori."', '".$proje."','".$il."','".$urun."','".$uygulama."','".$miktar."','tr',1)ON DUPLICATE KEY UPDATE ili = '".$il."', urun = '".$urun."', uygulama = '".$uygulama."', miktar = '".$miktar."', lang= 'tr', IsActive = 1");
	}
	else
	{	
  ...
  if (move_uploaded_file($_FILES["excel"]["tmp_name"], $target_file))
		{									   
			header("Location:../../ReferansListe.php");
