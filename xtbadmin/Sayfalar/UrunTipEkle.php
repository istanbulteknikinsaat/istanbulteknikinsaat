...
if(OturumAktif()==true)
{
	...
	if($_GET)
	{
		 if ($_SERVER['REQUEST_METHOD'] === 'GET') 
		 {
			...
			{
				$ID=$_GET['ID'];
				$Query=$db->prepare("SELECT * FROM uruntipleri WHERE IsActive=1 AND ID=?");
				$Query->execute(array($ID));
				$Row = $Query->fetch(PDO::FETCH_ASSOC);
				if($Row)
				{
					$KategoriAdi= $Row["UrunTipiAdi"];
					$Aciklama= $Row["UrunTipiAciklama"];
				}
			}
		}
	}
				...
				function Form1Kaydet() {
                    ...
                    if (Form1.KategoriAdi.value == "") {
                        BootstrapDialog.show({
                            ...
                            title: "Uyarı",
                            message: "Lütfen Ürün Tipi Adını Boş Bırakmayın..!"
                    ...
                    $.ajax(
                        {
                            url: "ajax/Kayit/UrunTipEkle.php",
                            type: "POST",
                            dataType: "JSON",
                                ...
                                if (hatavar) {
                                    ...
                                    BootstrapDialog.show({
                                        ...
                                        onhidden: function (dialogRef) {
                                            window.location="UrunTipListe.php";
                                        }
                                    });
                                }
                            },
