...
if(OturumAktif()==true)
{
	...
	if($_GET)
	{
		 if ($_SERVER['REQUEST_METHOD'] === 'GET') 
		 {
			...
			{
				$ID=$_GET['ID'];
				$Query=$db->prepare("SELECT * FROM urunuygulamalari WHERE ID=?");
				$Query->execute(array($ID));
				$Row = $Query->fetch(PDO::FETCH_ASSOC);
				if($Row)
				{
                    $UygulamaAdi= $Row["UrunTipiAdi"];
                    $UygulamaAdiEN= $Row["UrunTipiAdiEN"];
                    $UygulamaAdiDE= $Row["UrunTipiAdiDE"];
                    $UygulamaResim= $Row["UygulamaDetayResim"];
                    $UygulamaAciklama= $Row["UrunTipiAciklama"];
                    $UygulamaAciklamaEN= $Row["UrunTipiAciklamaEN"];
                    $UygulamaAciklamaDE= $Row["UrunTipiAciklamaDE"];
                    $lang= $Row["lang"];
            ...
            <div class="col-md-12">
                <div class="box box-success">
                    <div class="box-header">
                        <h3 class="box-title">Uygulama Ekle
                        </h3>
                        <!-- tools box -->
                                ...
                                <div class="col-xs-12">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="col-sm-4 control-label">Uygulama Adı</label>
                                            <div class="col-sm-8">
                                                <input type="text" name="UygulamaAdi" class="form-control input-sm "
                                                    value="<?= $UygulamaAdi ?>" placeholder="<?= $UygulamaAdi ?>">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-sm-4 control-label">Uygulama Adı EN</label>
                                            <div class="col-sm-8">
                                                <input type="text" name="UygulamaAdiEN" class="form-control input-sm "
                                                    value="<?= $UygulamaAdiEN ?>" placeholder="<?= $UygulamaAdiEN ?>">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-sm-4 control-label">Uygulama Adı DE</label>
                                            <div class="col-sm-8">
                                                <input type="text" name="UygulamaAdiDE" class="form-control input-sm "
                                                    value="<?= $UygulamaAdiDE ?>" placeholder="<?= $UygulamaAdiDE ?>">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-sm-4 control-label">Uygulama Aciklama</label>
                                            <div class="col-sm-8">
                                                <textarea class="ckeditor" id="UygulamaAciklamasiCK"
                                                    name="UygulamaAciklama"><?= $UygulamaAciklama ?></textarea>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-sm-4 control-label">Uygulama Aciklama EN</label>
                                            <div class="col-sm-8">
                                                <textarea class="ckeditor" id="UygulamaAciklamasiENCK"
                                                    name="UygulamaAciklamaEN"><?= $UygulamaAciklamaEN ?></textarea>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-sm-4 control-label">Uygulama Aciklama DE</label>
                                            <div class="col-sm-8">
                                                <textarea class="ckeditor" id="UygulamaAciklamasiDECK"
                                                    name="UygulamaAciklamaDE"><?= $UygulamaAciklamaDE ?></textarea>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="col-sm-4 control-label">Uygulama Resmi </label>
                                            <div class="col-sm-6">
                                                <label id="ResimSec"
...
<script>
    ...
    CKEDITOR.replace('UygulamaAciklamasiCK');
    CKEDITOR.replace('UygulamaAciklamasiENCK');
    CKEDITOR.replace('UygulamaAciklamasiDECK');


    function Form1Kaydet() {
        var Form1 = document.getElementById('Form1');
        if (Form1.UygulamaAciklama.value == "") {
        ...
        if (Form1.UygulamaAdi.value == "") {
            BootstrapDialog.show({
                type: BootstrapDialog.TYPE_WARNING,
                title: "Uyarı",
                message: "Lütfen Uygulama Adını Boş Bırakmayın..!"
        ...
        $.ajax({
            url: "ajax/Kayit/UygulamaEkle.php",
            type: "POST",
            dataType: "JSON",
                ...
                if (hatavar) {
                    ...
                    BootstrapDialog.show({
                        ...
                        onhidden: function (dialogRef) {
                            window.location = "UygulamaListe.php";
                        }
                    });
                }
            },
    ...
    function Form1Guncelle() {
        ...
        if (Form1.UygulamaAciklama.value == "") {
            BootstrapDialog.show({
                type: BootstrapDialog.TYPE_WARNING,
                title: "Uyarı",
                message: "Lütfen Sektörün Açıklamasını Boş Bırakmayın..!"
    ...
    $("#Kaydet").click(function () {
        for (var UygulamaAciklamasiCK in CKEDITOR.instances)
		        CKEDITOR.instances[UygulamaAciklamasiCK].updateElement();
        Form1Kaydet();
    });
    ...
    $("#Kaydet").click(function () {
        for (var UygulamaAciklamasiENCK in CKEDITOR.instances)
		        CKEDITOR.instances[UygulamaAciklamasiENCK].updateElement();
        Form1Kaydet();
    });
    $("#Duzenle").click(function () {
        for (var UygulamaAciklamasiDECK in CKEDITOR.instances)
		        CKEDITOR.instances[UygulamaAciklamasiDECK].updateElement();
        Form1Guncelle();
    });
    ...
    $(function () {
        ...
        var url = 'ajax/UrunUygulamaUpload/index.php',
            uploadButton = $('<label/>')
